...
| **`POST /api/integration/notify/registration`** *(envoi d'e-mails)* | prévient des personnes que leur **inscription est validée**. Corps : `{ recipients: [{ email, name? }], formTitle? }`. Envoie un e-mail « inscription validée » à chacun via le Gmail du club (`sendRegistrationConfirmedEmail`) et renvoie `{ sent, failed, errors }`. Déclenché par le desktop à la **validation** d'une préinscription. Nécessite `GMAIL_*` configuré |
