...
| **`POST /api/integration/notify/registration`** *(envoi d'e-mails)* | prévient des personnes que leur **inscription est validée**. Corps : `{ recipients: [{ email, name? }], formTitle? }` (le champ `name` est **ignoré**). Envoie **UN SEUL** e-mail « inscription validée », toutes les adresses en **copie cachée (BCC)**, **sans copie au club** (l'adresse d'envoi ne se reçoit pas le message à elle-même) — message commun, confidentialité préservée (`sendRegistrationConfirmedBulk`) — et renvoie `{ sent, failed, errors }` (`sent` = nb d'adresses si l'envoi a réussi). Déclenché par le desktop à la **validation** d'une préinscription. Nécessite `GMAIL_*` configuré |
